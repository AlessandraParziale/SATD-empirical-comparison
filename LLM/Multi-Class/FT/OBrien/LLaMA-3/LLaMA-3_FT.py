...
tracker = OfflineEmissionsTracker(
    ...
    experiment_id="Multi_Class_LLaMA-3_FT_Dataset_OBrien_All", on_csv_write="append")
tracker.start()
...
metrics_path = os.path.join(out_dir, "obrien_llama31-8b_lora_metrics.txt")
...
cm_csv = os.path.join(out_dir, "obrien_llama31-8b_lora_confmat.csv")
pd.DataFrame(cm, index=label_list, columns=label_list).to_csv(cm_csv, index=True)
print("saved confmat csv:", cm_csv)

pred_csv = os.path.join(out_dir, "obrien_llama31-8b_lora_preds.csv")
pd.DataFrame({
...
lora_dir = os.path.join(out_dir, "obrien_llama31-8b_lora_adapters")
