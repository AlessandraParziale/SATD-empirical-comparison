...
Choose and assign ONE label among: Code, M&T, Defect, Design, or Documentation.
""".strip()
...
# Dataset OBrien
# -----------------------------
df = pd.read_csv("OBrien.csv")

label_list = ["CODE", "M&T", "DEFECT", "DESIGN", "DOCUMENTATION"]

df["Is TD?"] = df["Is TD?"].astype(str).str.strip().str.upper()
df = df[df["Is TD?"] == "YES"].reset_index(drop=True)

df["label"] = df["Software TD Type"].astype(str).str.strip().str.upper()
df = df[df["label"].isin(label_list)].reset_index(drop=True)

df["comment_text"] = df["Comment Text"].astype(str).fillna("")
...
class_weights = torch.tensor(train_counts.sum() / train_counts, dtype=torch.float)
...
class T5WithLoRAForClassification(nn.Module):
    def __init__(self, checkpoint: str, num_labels: int, hidden_size: int, class_weights: torch.Tensor):
        super().__init__()
...
print("MAX_LEN:", MAX_LEN, "| EPOCHS:", EPOCHS, "| BATCH_SIZE:", BATCH_SIZE,
...
# Train
# =========================
model = T5WithLoRAForClassification(checkpoint, NUM_LABELS, HIDDEN_SIZE, class_weights).to(device)
...
tracker = OfflineEmissionsTracker(
    ...
    experiment_id="Multi_Class_Flan-t5-xl_FT_Dataset_OBrien_All", on_csv_write="append")
tracker.start()
...
os.makedirs("ft_satd_lora_results_V2", exist_ok=True)

metrics_path = "ft_satd_lora_results_V2/OBrien_flan-t5-xl_lora_metrics.txt"
...
cm_csv = "ft_satd_lora_results_V2/OBrien_flan-t5-xl_lora_confmat.csv"
pd.DataFrame(cm, index=label_list, columns=label_list).to_csv(cm_csv, index=True)
print("saved confmat csv:", cm_csv)


pred_csv = "ft_satd_lora_results_V2/OBrien_flan-t5-xl_lora_preds.csv"
pd.DataFrame({
...
lora_dir = "ft_satd_lora_results_V2/OBrien_flan-t5-xl_lora_adapters"
