...
if "Comment Text" not in df.columns:
    raise ValueError("Missing column: Comment Text")
if "Software TD Type" not in df.columns:
    raise ValueError("Missing column: Software TD Type")
if "Is TD?" not in df.columns:
    raise ValueError("Missing column: Is TD?")

df["Is TD?"] = df["Is TD?"].astype(str).str.strip().str.upper()
df = df[df["Is TD?"] == "YES"].reset_index(drop=True)

df["comment_text"] = df["Comment Text"].fillna("").astype(str)
df["label"] = df["Software TD Type"].astype(str).str.strip().str.upper()

labels = ["CODE", "M&T", "DEFECT", "DESIGN", "DOCUMENTATION"]
...
tracker = OfflineEmissionsTracker(
    ...
    experiment_id="Multi_Class_Flan-t5-xl_NO_FT_Dataset_OBrien_All", on_csv_write="append")
tracker.start()
...
with open(file_name + "_confmat.txt", "w") as output_file:
    ...
    for i, row in df.iterrows():
        ...
        pred_token = pred_token.replace(".", "").replace(",", "").replace(":", "").replace(";", "")
...
with open(file_name + "_confmat.txt", "a") as output_file:
