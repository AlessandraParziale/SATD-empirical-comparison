...
# Maldonado
# -----------------------------
df = pd.read_csv("maldonado_undersampled.csv")
df.columns = df.columns.str.strip() 

df["comment_text"] = df["comment_text"].astype(str).fillna("")
df["label"] = df["satd"].astype(int) 
...
test_df_original = test_df.reset_index(drop=True).copy()

dataset = {
    "Maldonado": DatasetDict({
...
tracker = OfflineEmissionsTracker(
    ...
    experiment_id="Binary_LLaMA-3_FT_Maldonado_undersampled", on_csv_write="append")
tracker.start()
...
for project_name, data in dataset.items():
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(test_prompts), BATCH_SIZE):
            ...
            preds_text = [normalize_prediction(p) for p in preds_raw]
            batch_preds_raw.extend(preds_raw)
            batch_preds_text.extend(preds_text)

            preds = [text_to_int(p) for p in preds_text]
        ...
        if epoch == num_epochs - 1:
            ...
            all_pred_raw = batch_preds_raw
            all_pred_text = batch_preds_text
...
test_with_predictions = test_df_original.copy()
test_with_predictions["pred_raw"] = all_pred_raw
test_with_predictions["pred_text"] = all_pred_text
test_with_predictions["predicted_label"] = all_pred

same_format_path = "results_all_LLAMA/maldonado_test_with_predictions.csv"
test_with_predictions.to_csv(same_format_path, index=False)

print(f"- Test (same format) + predictions: {same_format_path}")
