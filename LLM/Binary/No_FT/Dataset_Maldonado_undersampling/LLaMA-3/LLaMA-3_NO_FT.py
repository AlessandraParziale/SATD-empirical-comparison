...
tracker = OfflineEmissionsTracker(
    ...
    experiment_id="Binary_LLaMA-3_NO_FT_Dataset_Maldonado_undersampling", on_csv_write="append")




with open(file_name + "_confmat.txt", "w", encoding="utf-8") as output_file:
    output_file.write(f"Dataset: maldonado_undersampled.csv\n")
...
# Predictions CSV
orig_df = pd.read_csv("maldonado_undersampled.csv")
orig_df["predicted_label"] = all_pred
full_out_path = file_name + "_full_with_predictions.csv"
orig_df.to_csv(full_out_path, index=False, encoding="utf-8")
