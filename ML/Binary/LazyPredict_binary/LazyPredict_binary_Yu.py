...
os.environ['OPENBLAS_NUM_THREADS'] = '24'  # Limit OpenBLAS threads
...
# Remove classifiers that don't scale well with large datasets
excluded_classifiers = {
    'KNeighborsClassifier',           # Stores all training data
    'LabelPropagation',               # O(n²) memory for similarity matrices
    'LabelSpreading',                 # O(n²) memory for similarity matrices
    'QuadraticDiscriminantAnalysis',  # Computes covariance matrices per class
    'SVC'                             # Non-linear SVMs scale poorly
}
Supervised.CLASSIFIERS = [clf for clf in Supervised.CLASSIFIERS if clf[0] not in excluded_classifiers]
...
# Dataset: Maldonado
# -----------------------------
df = pd.read_csv("merged_Dataset_Yu.csv") 
