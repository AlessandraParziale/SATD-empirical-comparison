...
# Single model
# ----------------------------
def build_single_model(seed=6666):
...
def run_cv_and_save_binary(dataset_path,
    ...
    y = df["satd"].astype(int).values  # 0/1
    ...
    model = build_single_model(seed=seed)
...
tracker = OfflineEmissionsTracker(
    ...
    project_name="ML",
    experiment_id="Binary_RandomForestClassifier_Maldonado_undersampling",
    on_csv_write="append"
)
tracker.start()

run_cv_and_save_binary(
    dataset_path="maldonado_undersampled.csv",
    output_dir="results_satd_binary",
    n_splits=10,
    seed=6666,
    max_features=100
