...
def trainTestML(dataset):
    ...
    dataset["Is TD?"] = dataset["Is TD?"].astype(str).str.upper().str.strip()
    dataset = dataset[dataset["Is TD?"] == "YES"].copy()

    allowed = {"CODE", "M&T", "DEFECT", "DESIGN", "DOCUMENTATION"}
    dataset["Software TD Type"] = dataset["Software TD Type"].astype(str).str.upper().str.strip()
    dataset = dataset[dataset["Software TD Type"].isin(allowed)].copy()

    X_raw = dataset["Comment Text"].fillna("").astype(str).values
    y = dataset["Software TD Type"].astype(str).values 
